...
async function enrollFace(botName)
{
    ...
    do
    {
        ...
        if (name !== '')
        {
            await speak(botName, `Did you say you name is ${name}?`);
        ...
        if (success)
        {
            ...
            await speak(botName, `Please, look a the camera. I will memorise your face`);
            let enrolled = await learnFace(name, botName);
            if (enrolled)
                await speak(botName, `I memorised your identity`);
            else
                await speak(botName, `I have not memorised your identity`);
        }
        else
        {
            name = '';
            await speak(botName, `Sorry, I have not understood your name. What's your name?`);
        }
    }while (name === '');
}
...
let subHandleEngagement = manager.subscribe('user_engagement',
    async (msg) => 
    {
        if (!isEngaging)
        {
            ...
            if (result !== false)
                await greetUser("Ciao", result, botName);
            else
